...
For that purpose, we have to set up an object, which that contains information about which variable we want to shock (`shock`), the size of the shock (`scale`) and whether it is a shock applied in a single country or in multiple countries (`global`). We can use the helper function `get_shockinfo` to set up a such a dummy object which we can subsequentially modify according to our needs. The following lines of code are used for a negative 100 bp shock applied to US short term interest rates:

 </p>
```{r "us.mp", results="hide"}
# US monetary policy shock
shockinfo<-get_shockinfo("chol")
shockinfo$shock<-"US.stir"
shockinfo$scale<--100
irf.chol.us.mp<-irf(model.ssvs.1, n.ahead=24, ident="chol", shockinfo=shockinfo, expert=list(save.store=TRUE))
```
...
plot(irf.sign, resp="US.y", shock="US.Dp")
...
matplot(cbind(irf.sign$IRF_store[1,"US.y_t+4",1,],
              irf.sign$IRF_store[1,"US.y",1,]),
        type="l",ylab="",main="y",lwd=2,xaxt="n")
axis(side=1,at=c(1:5,9,13,17,21,25),label=c(0:4,8,12,16,20,24))
legend("topright",lty=c(1,2),c("expected","actual"),lwd=2,bty="n",col=c("black","red"))
yy<-irf.sign$IRF_store[1,"US.y_t+4",1,1]
...
plot(irf.sign.ssvs, resp=c("AT.y","BE.y","DE.y","ES.y"), shock="Global.ltir")
```
