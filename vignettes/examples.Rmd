...
<p align="justify">
Now we can re-estimate the model by specifying `OE.weights`, which is a list with its entries labelled exactly like the additional country models. 
</p>
...
For that purpose, we have to set up an `shockinfo` object, which contains information about which variable we want to shock (`shock`), the size of the shock (`scale`) and whether it is a shock applied in a single country or in multiple countries (`global`). We can use the helper function `get_shockinfo()` to set up a such a dummy object which we can subsequently modify according to our needs. The `irf()` command needs as input an estimated model object, the impulse response horizon (`n.ahead`), the preferred identificaiton scheme (`ident`) and the `shockinfo` dataframe. For the recursive identification and GIRFs the shockinfo argument is actually optional. In case this object is not provided, the model retrieves all shocks with unit scaling. The following lines of code are used for a negative 100 bp shock applied to US short term interest rates:
...
The `shockinfo` argument for the arguments `chol` and `girf` look exactly the same. If we compare them, we notice that both have three columns defining the shock, the scale and whether it is defined as global shock.
...
```
The results are stored in `irf.chol.us.mp`. In order to save the complete store one can activate the `save.store` argument by setting to `TRUE` within the expert settings (Attention: this may need a lot of memory).
...
We specify the sign restrictions by specfying again a `shockinfo` argument for the `irf()` command. For the case of sign-restrictions, the user has to specify a `shockinfo` argument otherwise no signs are provided for the computation of the impulse responses. Hence, we create an object with `get_shockinfo("sign")` now and with the help of another function `add_shockinof()`, we can add sign restrictions. In particular, we can specify single signs or a set of signs for one particular structural shock at a time. Information is needed which variable to shock, on which IRFs to put the sign restrictions, the restrictions and the horizon how long these restrictions should hold. Increasing the number of restrictions (on the variables or the horizon) will lead to preciser inference; however, finding a suitable rotation matrix will become substantially harder. 
</p>
...
plot(irf.sign, resp="US.y", shock="US.y")
plot(irf.sign, resp="US.Dp", shock="US.Dp")
plot(irf.sign, resp=c("US.Dp","US.rer"), shock="US.y")
plot(irf.sign, resp=c("US.Dp","US.rer"), shock="US.Dp")
```
...
* `ident`: Character defining type of identification scheme. Default `chol` refers to zero restrictions via the Cholesky decomposition while `sign` refers to sign-restrictions and `girf` to generalized impulse responses.
...
* `expert` Expert settings, must be provided as list. Default is set to `NULL`.
    ...
    + `use_R` Boolean whtether IRF computation should fall back on `R` version, otherwise `Rcpp` version is used (default).
