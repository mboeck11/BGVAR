...
struct IrfParallel : public RcppParallel::Worker
{
  ...
  IrfParallel(cube A_large, cube S_large, cube Ginv_large, List shocklist, int type, int nhor, int thindraws, Function Rchol, 
              NumericMatrix irf_output, NumericMatrix rot_output, IntegerMatrix counter_output) 
    : A_large(A_large), S_large(S_large), Ginv_large(Ginv_large), shocklist(shocklist), type(type), nhor(nhor), thindraws(thindraws), 
  ...
  void operator()(std::size_t begin, std::size_t end) {
    ...
    // current draw
    mat Amat = A_large.row(begin);
    mat Smat = S_large.row(begin);
    mat Ginv = Ginv_large.row(begin);
    // construct Fmat
    ...
    if(chol_success){
      ...
      if(type==3){
        ...
        while( condall == 0.0 && icounter < MaxTries){
            ...
            if(shock_cidx[cc]){
              ...
              if(nozero[cc]){
                ...
                for(uword i = 0; i < Kidx; i++){
                  ...
                  colvec Zsum = sum(Ztemp,1); uvec zidx = find(Zsum);
          ...
          for(uword kk = 0; kk < bigK; kk++){
            ...
            if(sum(abs(STemp))>0){
              ...
              double chksum = arma::as_scalar(sum(sign(PDiag * STemp)));
        ...
        // save counter
        counter_output(begin,0) = icounter;
      }else{
        counter_output(begin,0) = 1;
      }
      // compute shock
      ...
      for(size_t ihor = 0; ihor < nhor; ihor++){
        const size_t ihor_begin = begin*chunksize + bigK*ihor;
        const size_t rot_begin = begin*bigK;
...
List compute_irf_parallel(arma::cube A_large, arma::cube S_large, arma::cube Ginv_large, const int type, const int nhor, const int thindraws, const SEXP shocklist_in) {
  ...
  // set up worker
  IrfParallel irf(A_large, S_large, Ginv_large, shocklist, type, nhor, thindraws, Rchol, irf_output, rot_output, counter_output);
  // call parallelFor to do the work
  RcppParallel::parallelFor(0, thindraws, irf, 1);
...
List compute_irf(arma::cube A_large, arma::cube S_large, arma::cube Ginv_large, const int type, const int nhor, const int thindraws, const SEXP shocklist_in) {
  ...
  //----------------------------------------------------------------------------
  //int begin = 0; 
  for(uword begin = 0; begin < thindraws; begin++){
    // current draw
    mat Amat = A_large.row(begin);
    mat Smat = S_large.row(begin);
    mat Ginv = Ginv_large.row(begin);
